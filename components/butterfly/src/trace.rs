    ...
    (GOSSIP: $server:expr, $msg_type:expr, $to_member_id:expr, $payload:expr) => {{
        ...
        if trace_on {
            ...
            let rp = match $payload.payload {
                ...
                rumor::RumorPayload::Service(ref service) => format!(
                    "{}-{}-{}",
                    service.member_id, service.service_group, service.incarnation
                ),
                rumor::RumorPayload::ServiceConfig(ref service_config) => format!(
                    "{}-{}-{}",
                    service_config.service_group,
                    service_config.incarnation,
                    service_config.encrypted
                ),
                rumor::RumorPayload::ServiceFile(ref service_file) => format!(
                    "{}-{}-{}-{}",
                ...
                rumor::RumorPayload::Election(ref election) => format!(
                    "{}-{}-{}-{}-{:?}-{:?}",
                    election.member_id,
                ...
                rumor::RumorPayload::Departure(ref departure) => format!("{}", departure.member_id),
            };
